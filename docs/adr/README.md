...
| ADR-057 | The upgrade contract: an upgrade changes the VeloxSearch control plane (and ADR-055's fields), never the operator, its CRDs or `OpenSearchCluster` specs — bootstrap installs only what is absent, operator drift is reported (R9), upgrades apply `upgrade.yaml` (no bootstrap binding) and a re-applied binding is revoked again |
