...
| **R3** | **Flexible deployment storage** (ADR-061, ratified 2026-09-20; amends ADR-043, which amended ADR-031): the `longhorn` StorageClass (provisioner `driver.longhorn.io`) when present; otherwise the cluster's own default StorageClass; Longhorn auto-bootstrap only when there is no default at all | OpenSearch nodes claim PVCs (ADR-031). Longhorn remains the preferred path: when its SC exists, PVCs are pinned to it and replica sizing is reconciled (`#26`). A **foreign CSI default** (EBS/gp2, Ceph…) is accepted as durable — PVCs omit `storageClassName` and ride it. A **node-local default** (`rancher.io/local-path`, hostpath, `kubernetes.io/no-provisioner`) is accepted with a durability warning (data lost on pod reschedule) so day-0 evaluation on a stock k3s doesn't require a storage stack | List `storageclasses`; check for the `longhorn` SC **by name + provisioner**, else classify the annotated default | Warn/pass (never refuse a usable default): Longhorn → pass. Foreign CSI default → pass ("accepted — ADR-043 amended"). Node-local → warn ("data lost on reschedule; install Longhorn for HA storage"). No default SC → VeloxSearch auto-installs Longhorn (install + inform, never ask); nodes missing prerequisite packages (`open-iscsi`, nfs client, `dmsetup`) are surfaced **in-app, per node, with per-distro (Debian/Ubuntu/Arch) install commands**, and creation waits until Longhorn is usable |
...
| **R9** | The running OpenSearch operator is the one this release vendors (**informational**) | Bootstrap installs only what is absent and never replaces a running operator, so a release that vendors a different operator leaves the installed one in place (ADR-057). The difference is worth seeing | Image of our operator's controller Deployment vs the image in `deploy/bootstrap/operator.yaml` | Never fails. Warn: "operator drift: running X, this release vendors Y" with the remediation in DEPLOY.md, "Operator version drift". Not installed / not ours: pass |
...
> **Storage contract change (2026-09-20, ADR-061).** The per-fixture storage
> behaviour in the table above reflects runs under the pre-ADR-061 contract
> (Longhorn-only). From ADR-061 on, `k3s-greenfield` (local-path default)
> creates deployments directly on the cluster default with a durability
> warning — no Longhorn install; `k0s-bare` (no default SC) still triggers the
> Longhorn bootstrap; the 3-node `longhorn` fixture is unchanged.
